...
# Модель, возвращающая значения активаций вскх целевых слоев
feature_extractor = keras.Model(inputs=model.inputs, outputs=outputs_dict)

# Определим функцию потерь содержимого, которая позволит гарантировать сходство представлений целевого и сгенерированного изображений в верхнем слое сети
...
content_weight: float = 2.5e-8 # Вес вклада потери содержимого
...
def compute_loss(combination_image: tf.Tensor, base_image: tf.Tensor, style_reference_image: tf.Tensor) -> tf.Tensor:
    ...
    Вычисляет общую функцию потерь
    """
...
def compute_loss_and_grads(combination_image: tf.Tensor, base_image: tf.Tensor, style_reference_image: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
    """
    Вычисляет общую функцию потерь и градиенты
    """
...
# Создаем оптимизатор градиентного спуска с затуханием
...
combination_image: tf.Variable = tf.Variable(preprocess_image(base_image_path)) # сгенерированное изображение
